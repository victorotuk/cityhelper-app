...
const STEPS = [
  ...
  },
  {
    ...
    icon: <Bell size={24} />,
    title: 'Stay Notified',
    description: 'Push notifications are on by default. You\'re all set to never miss a deadline!',
    isLast: true,
  },
];
...
export default function WelcomeGuide({ userId, onComplete }) {
  ...
  const handleNext = async () => {
    if (isLastStep) {
      // Enable push silently and close
      await requestPushPermission(userId).catch(() => {});
      handleComplete();
    } else {
